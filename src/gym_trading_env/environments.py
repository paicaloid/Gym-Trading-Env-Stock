...
class TradingEnv(gym.Env):
    ...
    def __init__(self,
        ...
        self.log_metrics = []
        self.upper_lower_multipliers = upper_lower_multipliers
    ...
    def _triple_barrier(self):
        prices = self.df.iloc[self._idx]
        # cal volatility
        vol = self.df["close"].pct_change()
        vol = vol.ewm(span=20).std()
        vol.dropna(inplace=True)
        # set the top barrier
        barriers = pd.DataFrame(columns=['top_barrier', 'bottom_barrier'],
                                index=vol.index)
        if self.upper_lower_multipliers[0] > 0:
            top_barrier = prices['close'] + prices['close'] * \
                self.upper_lower_multipliers[0] * vol.iloc[self._idx]
        else:
            # set it to NaNs
            top_barrier = pd.Series(index=prices['close'].index)
        # set the bottom barrier
        if self.upper_lower_multipliers[1] > 0:
            bottom_barrier = prices['close'] - \
                prices['close'] * self.upper_lower_multipliers[1] * vol.iloc[self._idx]
        else:
            # set it to NaNs
            bottom_barrier = pd.Series(index=prices['close'].index)
        barriers.loc[:, ['top_barrier', 'bottom_barrier']] = \
